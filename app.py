...
st.title("🤝 Multi-Agent RAG — Salary & Insurance (Gemini)")
...
for msg in st.session_state.messages:
    ...
    with st.chat_message(role):
        if meta.get("route"):
            badge = "🧭 Coordinator → " + ("💼 Salary" if meta["route"] == "salary" else "🛡 Insurance")
...
if user_query:
    ...
    with st.chat_message("assistant"):
        st.markdown(f"<small>🧭 Coordinator → {'💼 Salary' if route=='salary' else '🛡 Insurance'}</small>", unsafe_allow_html=True)
        st.write(answer)
